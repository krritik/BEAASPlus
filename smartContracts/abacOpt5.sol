// This is the most latest version of Smart contract with all recent and new functions
...
contract ABACOpt5 {
    ...
    // new event logs
    event accessMultipleResponse(uint256 indexed start, uint256 indexed end, uint256[] idx, bytes32[] userId, bytes32[] objectId, bytes32[] operation, bool[] response); 
    ...
    function getMultipleResponse(uint256 _start, uint256 _end, uint256[] calldata _idx, bytes32[] calldata _userId, bytes32[] calldata _objectId, bytes32[] calldata _operation, bool[] calldata _response) external {
        emit accessMultipleResponse(_start, _end, _idx, _userId, _objectId, _operation, _response);
    }
}